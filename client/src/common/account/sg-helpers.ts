...
async function getSgQueryHelpers(rpc: string) {
  ...
  async function getAllBalances(address: string, isDisplayed: boolean = false) {
    const res = await bankExtension.bank.allBalances(address);
    return logAndReturn(res, isDisplayed);
  }

  async function getMetadata(denom: string, isDisplayed: boolean = false) {
    const res = await bankExtension.bank.denomMetadata(denom);
    return logAndReturn(res, isDisplayed);
  }

  async function getValidators(isDisplayed: boolean = false) {
    const { validators: res } = await stakingExtension.staking.validators(
      "BOND_STATUS_BONDED"
    );
  ...
  async function getTimeInNanos(isDisplayed: boolean = false) {
    const {
      block: {
        header: { time },
      },
